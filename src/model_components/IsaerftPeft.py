...
class IsaerftModel(nn.Module):
    ...
    def setup_trainable_blocks(self):
        ...
        # First, find all matching SAEs across all patterns
        ...
        for sae in all_matching_saes:
            ...
            feature_dim = sae.d_sae
            
            # Create a ResidualBlock based on the config
            self.trainable_blocks[sae.name] = ResidualBlock(
                input_dim=feature_dim,
                hidden_layers=self.peft_config[self.active_adapter].depth,
                hidden_dim=self.peft_config[self.active_adapter].hidden_size,
                name=f"residual_block_{sae.name}"
    ...
    def add_hooks(self):
        ...
        for hook_name, sae in self.saes.items():
            ...
            trainable_hook = partial(
                resid_hook, 
                residual_block=self.trainable_blocks[hook_name]
            )
...
class IsaerftPeft(PeftModel):
    ...
    def get_trainable_parameters(self):
        """Get the trainable parameters of the model"""
        return self.active_adapter_module.get_trainable_parameters()
...
if __name__ == "__main__":
    ...
    config = IsaerftConfig(target_hooks='layer_25/width_16k/canonical', depth=-1)

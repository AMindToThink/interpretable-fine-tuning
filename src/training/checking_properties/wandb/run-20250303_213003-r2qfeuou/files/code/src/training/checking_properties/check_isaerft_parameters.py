...
def check_parameter_changes_with_trainer(model, tokenizer):
    ...
    # Use a simple dataset and preprocess it to include input_ids
    dataset = load_dataset("stanfordnlp/imdb", split="train").select(range(5))
    
    # Preprocess the dataset to include input_ids
    def preprocess_function(examples):
        # Use the text field from IMDB
        return tokenizer(examples["text"], truncation=True, padding="max_length", max_length=512)
    
    # Apply preprocessing to add input_ids
    tokenized_dataset = dataset.map(preprocess_function, batched=True)
    
    # Make sure the dataset has the required format
    print("Dataset features:", tokenized_dataset.features)
    print("First example:", tokenized_dataset[0])

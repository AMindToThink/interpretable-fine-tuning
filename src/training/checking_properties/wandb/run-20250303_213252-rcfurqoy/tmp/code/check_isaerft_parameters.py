...
def check_parameter_changes_with_trainer(model, tokenizer):
    ...
    # Load the same dataset used in the ORPO trainer
    dataset = load_dataset(path="trl-lib/ultrafeedback_binarized", split="train").select(range(5))
    
    # Print dataset structure to understand what we're working with
    ...
    training_args = SFTConfig(
        ...
        learning_rate=1.0,  # High learning rate to ensure changes
        no_cuda=True,
        max_steps=3,
    ...
    # Create the trainer - using the same approach as in ORPO
    trainer = SFTTrainer(
        model=model_cpu,
        args=training_args,
        train_dataset=dataset,
        processing_class=tokenizer,
        # The dataset already has the right format with chosen/rejected responses
    )

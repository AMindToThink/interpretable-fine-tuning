...
def check_parameter_changes_with_trainer(model, tokenizer):
    ...
    # Set up a minimal training run
    training_args = TrainingArguments(
        output_dir="./test-trainer",
        num_train_epochs=1,
        per_device_train_batch_size=1,
        gradient_accumulation_steps=1,
        max_steps=1,  # Just one step to see parameter changes
        save_steps=10,
        logging_steps=1,
        learning_rate=1e-4,
    ...
    sample_text = "This is a test sentence for fine-tuning."
    ...
    class SimpleDataset(torch.utils.data.Dataset):
        ...
        def __getitem__(self, idx):
            item = {key: val[0] for key, val in self.inputs.items()}
    ...
    for name, param in model.named_parameters():
        ...
        if name in initial_params:
            diff = torch.sum(torch.abs(param.detach().cpu() - initial_params[name])).item()
            if diff > 0:

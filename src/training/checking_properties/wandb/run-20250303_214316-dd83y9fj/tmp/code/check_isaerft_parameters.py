...
def check_parameter_changes_with_trainer(model, tokenizer):
    ...
    # Load the dataset
    dataset = load_dataset("stanfordnlp/imdb", split="train").select(range(3))

    # Print dataset structure
    print("Dataset features:", dataset.features)
    print("Sample data:", dataset[0])
    
    # Create a properly formatted dataset with input_ids
    def preprocess_function(examples):
        # Create prompts from the examples
    ...
    # Create the trainer with pre-tokenized dataset
    trainer = SFTTrainer(
        model=model_cpu,
        args=training_args,

# Todo: make the dataclass extend PeftConfig and work out pushing the model to hub
...
class IsaerftConfig():#(PeftConfig):
    ...
    """
    target_hooks:Union[List[str], str] = field()
    ...
    hidden_size:Optional[int] = field(default=128, metadata={'help':'The hidden size of the FFNN. depth must be > 0 for this to be provided.'})
    lora_r:Optional[int] = field(default=64, metadata={'help':'The rank of the matrices for the FFNN. Depth must be greater than -1.'}) # Todo: find an actually good lora_r value here
    def __post_init__(self):
        # super().__post_init__() include once extending PeftConfig
